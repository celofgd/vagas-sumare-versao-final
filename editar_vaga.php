<?php
/**
 * Formulário para o recrutador editar uma vaga que ele mesmo criou.
 */
...
$erros = [];
$idEdicao = isset($_GET['edit_id']) ? (int) $_GET['edit_id'] : 0;

$usuario = usuario_atual();
$userId = (int) $usuario['id'];

// Só permite editar se a vaga foi criada por este usuário
$consulta = $pdo->prepare('SELECT * FROM vagas WHERE id = ? AND criado_por = ?');
$consulta->execute([$idEdicao, $userId]);
...
if (!isset($vaga['nao_possui_endereco'])) {
    $vaga['nao_possui_endereco'] = (trim($vaga['rua'] ?? '') === '' && trim($vaga['cidade'] ?? '') === '');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($erros)) {
        ...
        $consulta = $pdo->prepare('UPDATE vagas SET titulo = ?, empresa = ?, localizacao = ?, rua = ?, numero = ?, bairro = ?, cidade = ?, cep = ?, descricao = ?, salario = ?, regime = ?, dia_inicio = ?, dia_fim = ?, horario_inicio = ?, horario_fim = ?, beneficios = ?, meio_contato = ? WHERE id = ?');
        $consulta->execute([
            ...
            $beneficios ?: null, $meio_contato, $idVaga
        ]);
        header('Location: gerenciar_vagas.php?msg=Vaga+atualizada+com+sucesso');
        exit;
    }
    $vaga = [
        'id' => $idVaga,
...
require_once 'templates/view_editar_vaga.php';
